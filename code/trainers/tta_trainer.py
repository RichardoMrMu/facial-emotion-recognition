...
from utils.emo_loss import soft_loss,FocalLoss
...
class FER2013Trainer(Trainer):
    ...
    def __init__(self, model, train_set, val_set, test_set, configs):
        ...
        if configs['type'] == 0:
            ...
            self._model = model()
            if configs['emo_name'] == "crossformer_b":
                checkpoint = torch.load("/home/tbw/work/code/ResidualMasking/pretrain/crossformer-b.pth",
                                        map_location='cpu')
                msg = self._model.load_state_dict(checkpoint['model'], strict=False)
                print(msg)
            print(self._model)
        ...
        elif self._configs['loss']=='cross':
            self._criterion = nn.CrossEntropyLoss(class_weights).to(self._device)
        else:

            self._criterion = FocalLoss().to(self._device)
        if self._configs['optimizer'] == 'radam':
            self._optimizer = RAdam(
                params=self._model.parameters(),
                lr=self._lr,
                weight_decay=self._weight_decay,
            )
        elif self._configs['optimizer'] == 'sgd':
            self._optimizer = SGD(
                params=self._model.parameters(),
                lr=self._lr,
                momentum=0.9,
        ...
        elif self._configs['optimizer'] == 'adam':
            self._optimizer = torch.optim.Adam(
                ...
                weight_decay=self._weight_decay,
                amsgrad=False)
        elif self._configs['optimizer'] == 'adamw':
            ...
            raise ValueError
        if self._configs['scheduler'] == "cosine":
            self._scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
                self._optimizer,
                T_max=self._configs['T_max'],
                eta_min=1e-6,
                verbose=True,
            )
        elif self._configs['scheduler'] == "reduce":
        ...
        elif self._configs['scheduler'] == "exp":
            self._scheduler = torch.optim.lr_scheduler.ExponentialLR(
                                self._optimizer,
                                gamma=0.95)
        elif self._configs['scheduler'] == "step":
            self._scheduler = torch.optim.lr_scheduler.StepLR(
                self._optimizer,
                step_size=self._configs['stepsize'],
                gamma=0.1,
                verbose=False,
            )
        ...
        self._val_acc_list = []
        self._test_acc_list = []
        # self._test_losss_list = []
        self._best_test_loss = 1e9
    ...
    def _train(self):
        ...
        for i, (images, targets) in tqdm(
            ...
            one_hot_label = (1 - 0.7) * F.one_hot(targets, num_classes=7) + 0.7 / 7
            # compute output, measure accuracy and record loss
            outputs = self._model(images)

            if self._configs['loss'] == "cross":
                loss = self._criterion(outputs, targets)
            elif self._configs['loss'] == 'focalloss':
                loss = self._criterion(outputs, targets) + soft_loss(outputs, one_hot_label)
    ...
    def _test_with_tta(self):
        ...
        with torch.no_grad():
            ...
            i += 1
            # self._test_loss_list.append(val_loss / i)
            self._test_acc_list.append(test_acc / i)
    ...
    def _calc_acc_on_private_test_with_tta(self):
        ...
        logger.info("Calc acc on private test with tta..")
        f = open(
        ...
        logger.info("Accuracy on private test with tta: {:.3f}".format(test_acc))
        f.close()
        return test_acc
    ...
    def train(self,gpu):
        ...
        # exit(0)
        if self._distributed == 1:
            torch.distributed.init_process_group(backend="nccl", init_method='tcp://localhost:23456', rank=gpu,
                                                 world_size=4*64)
        try:
    ...
    def _update_training_state(self):
        ...
        if self._configs['scheduler'] == 'reduce':
